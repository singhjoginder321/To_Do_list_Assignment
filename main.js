    ...
    if (item.classList[0] === 'edit-btn') {
        ...
        input.addEventListener('blur', () => {
            ...
            if (updatedText) {
                ...
                updateLocalTodos(item.parentElement, updatedText);
            } else {
                input.replaceWith(todoItem);
            }
        });
...
function updateLocalTodos(todoElement, updatedText) {
    let todos;
    if (localStorage.getItem('todos') === null) {
        todos = [];
    } else {
    ...
    const todoIndex = todos.indexOf(todoElement.children[0].innerText);
